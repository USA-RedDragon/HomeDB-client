isWindows=-1<navigator.platform.indexOf('Win'), isWindows?($('.sidebar .sidebar-wrapper, .main-panel').perfectScrollbar(), $('html').addClass('perfect-scrollbar-on')):$('html').addClass('perfect-scrollbar-off'); const breakCards=!0; const searchVisible=0; let transparent=!0; const transparentDemo=!0; const fixedTop=!1; let mobile_menu_visible=0; let mobile_menu_initialized=!1; const toggle_initialized=!1; const bootstrap_nav_initialized=!1; let seq=0; const delays=80; const durations=500; let seq2=0; const delays2=80; const durations2=500; function debounce(n, i, t) {
    let r; return function() {
        const e=this; const a=arguments; clearTimeout(r), r=setTimeout(function() {
    ...
    if ($toggle=$(this), 1==mobile_menu_visible) {
        ...
        }, 430); const e=$('<div class="close-layer"></div>'); 0!=$('body').find('.main-panel').length?e.appendTo('.main-panel'):$('body').hasClass('off-canvas-sidebar')&&e.appendTo('.wrapper-full-page'), setTimeout(function() {
            e.addClass('visible');
...
}), md={ misc: { navbar_menu_visible: 0, active_collapse: !0, disabled_collapse_init: 0 }, checkSidebarImage: function() {
    ...
    if (0!=$('#dailySalesChart').length||0!=$('#completedTasksChart').length||0!=$('#websiteViewsChart').length) {
        dataDailySalesChart={ labels: ['M', 'T', 'W', 'T', 'F', 'S', 'S'], series: [[12, 17, 7, 17, 23, 18, 38]] }, optionsDailySalesChart={ lineSmooth: Chartist.Interpolation.cardinal({ tension: 0 }), low: 0, high: 50, chartPadding: { top: 0, right: 0, bottom: 0, left: 0 } }; const e=new Chartist.Line('#dailySalesChart', dataDailySalesChart, optionsDailySalesChart); md.startAnimationForLineChart(e), dataCompletedTasksChart={ labels: ['12p', '3p', '6p', '9p', '12p', '3a', '6a', '9a'], series: [[230, 750, 450, 300, 280, 240, 200, 190]] }, optionsCompletedTasksChart={ lineSmooth: Chartist.Interpolation.cardinal({ tension: 0 }), low: 0, high: 1e3, chartPadding: { top: 0, right: 0, bottom: 0, left: 0 } }; const a=new Chartist.Line('#completedTasksChart', dataCompletedTasksChart, optionsCompletedTasksChart); md.startAnimationForLineChart(a); const n=Chartist.Bar('#websiteViewsChart', { labels: ['J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D'], series: [[542, 443, 320, 780, 553, 453, 326, 434, 568, 610, 756, 895]] }, { axisX: { showGrid: !1 }, low: 0, high: 1e3, chartPadding: { top: 0, right: 5, bottom: 0, left: 0 } }, [['screen and (max-width: 640px)', { seriesBarDistance: 5, axisX: { labelInterpolationFnc: function(e) {
            return e[0];
        } } }]]); md.startAnimationForBarChart(n);
    }
}, showNotification: function(e, a) {
    type=['', 'info', 'danger', 'success', 'warning', 'primary'], color=Math.floor(5*Math.random()+1), $.notify({ icon: 'add_alert', message: 'Welcome to <b>Material Dashboard</b> - a beautiful freebie for every web developer.' }, { type: type[color], timer: 3e3, placement: { from: e, align: a } });
